#!/usr/bin/env python3
"""Corporate PPT renderer (HTML-first pipeline) â EDUCA EDTECH Group.
...
Each slide renders as real HTML/CSS (same design system as the
corporate-ppt Artifact tool: slides.css + icons.py + templates.py),
...
# Reuse the brand fonts already committed under server/fonts/ instead of
# duplicating the binary .ttf files inside the skill directory.
FONTS_DIR = os.path.join(HERE, "..", "..", "..", "..", "server", "fonts")
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python3 render.py plan.json output.pptx", file=sys.stderr)
        sys.exit(1)
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        assemble_pptx(plan, tmp_dir, out_path)
    ...
    print(f"saved {out_path} ({os.path.getsize(out_path)} bytes, {len(plan['slides'])} slides)")
