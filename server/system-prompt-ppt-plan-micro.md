...
| `icon_grid` | 3–6 concepts/dimensions, each with an icon (2-column grid) — default for "N cards" content, e.g. the four dimensions of maturity | `{ "items": [{ "icon": "dns", "title": "Infrastructure", "text": "short description" }] }` — `icon` optional |
