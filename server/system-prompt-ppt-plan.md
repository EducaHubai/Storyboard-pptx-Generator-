...
| `illustration` | A decorative hero image for entrada (welcome/hook) only — NOT resumen/título/cierre (all stay minimal per §7), and not a substitute for a conceptos/puntos_clave diagram | `{ "name": "one of the illustration names below", "accentColor": "hex, optional — defaults to teal" }` |
