#!/usr/bin/env python3
"""Corporate PPT renderer â "epigrafe" format (HTML-first pipeline).

Usage: python3 render.py <input.json> <output.pptx>

input.json shape (mirrors render_ppt.py's convention so index.js's
/api/ppt-generate flow works unchanged, just pointed at this script):
{
  "plan": {
    "unit": "...", "epigrafe": "...", "afo": "...",
    "slides": [
      {"n": 1, "section": "titulo", "fields": {"title": "..."}},
      {"n": 2, "section": "inicio", "fields": {"icon": "lightbulb", "promise": "..."}},
      {"n": 3, "section": "concepto", "variant": "numero_hero", "fields": {...}},
      {"n": 4, "section": "puntos_clave", "variant": "flujo_pasos", "fields": {...}},
      {"n": 5, "section": "resumen", "fields": {"title": "...", "items": [...]}},
      {"n": 6, "section": "cierre", "fields": {"title": "Thank you"}}
    ]
  },
  "scripts": [
    {"n": 1, "script": "...", "productionNotes": "..."}
  ]
}

Each slide renders as real HTML/CSS (icons.py + templates.py + slides.css â
...
as REAL editable pptx text boxes overlaid on top â only decorative art
(the giant hero number, card shadows, gradients) is baked into the
background image. Roboto/Lato are embedded into the .pptx afterward so
...
def assemble_pptx(plan, scripts_by_n, tmp_dir, out_path):
    prs = Presentation()
...
def embed_fonts(pptx_path):
    ...
    correctly even without those fonts installed locally (same technique
    as index.js's embedFonts(), ported to Python/zipfile)."""
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        assemble_pptx(plan, scripts_by_n, tmp_dir, output_path)
