...
function buildMockPptPlan({ epigraphs, unit, afo, avatar }) {
  const slides = [
    { n: 1, section: "title", title: unit, subtitle: afo, bullets: [], graphic: "none" },
    { n: 2, section: "entrada", title: "What you will learn", bullets: ["Overview of key concepts", "Practical applications", "Tools and frameworks"], graphic: "none" },
  ];
  epigraphs.slice(0, 3).forEach((ep, i) => {
    slides.push({ n: slides.length + 1, section: "conceptos", title: ep, bullets: ["Core idea one", "Core idea two", "Core idea three"], graphic: "diagram or flow" });
  });
  epigraphs.slice(3, Math.min(epigraphs.length, 6)).forEach((ep, i) => {
    slides.push({ n: slides.length + 1, section: "puntos_clave", title: ep, bullets: ["Key takeaway one", "Key takeaway two"], graphic: "none" });
  });
  const total = slides.length + 2;
  slides.push({ n: total - 1, section: "resumen", title: "Summary", bullets: ["Main idea recap", "Second key point", "What comes next"], graphic: "none" });
  slides.push({ n: total, section: "cierre", title: "Now it's your turn", bullets: ["Complete the activity in this unit", "Apply what you've learned"], graphic: "none" });
...
function buildMockPptResult(plan) {
  ...
  return { educalab, scriptSlides, blobUrl, fileName: `${plan.unit.toLowerCase().replace(/[^a-z0-9]+/g, "-")}.pptx`, plan };
}
...
export default function App() {
  ...
  const handlePptGenerate = useCallback(async (approvedPlan) => {
    ...
    try {
      ...
      const fileName = `${slugify(approvedPlan.unit || "ppt")}.pptx`;
    ...
    <div style={styles.app}>
      ...
      <main style={styles.main}>
        ...
        {pptScreen === PPT_SCREENS.INPUT && (
          <UploadScreen onUpload={handlePdfUpload} loading={loading} />
        )}
  ...
  return (
    <div style={styles.centeredScreen}>
      <div style={styles.heroBlock}>
        ...
        <h1 style={styles.h1}>Upload the course unit PDF</h1>
        <p style={styles.lead}>
          Drop the temario or training material PDF. Claude will extract the content,
          propose an 8–10 slide plan, and wait for your approval before generating anything.
        </p>
      </div>
  ...
  return (
    <div style={styles.wideScreen}>
      <div style={styles.screenHeader}>
        ...
        <p style={styles.lead}>{plan.afo} · {plan.totalSlides} slides · ~{plan.estimatedDuration}</p>
        <p style={{ ...styles.lead, fontSize: 13, marginTop: 8 }}>Edit titles and bullets below, then approve to generate the .pptx + script + EducaLab metadata.</p>
      </div>
          ...
          return (
            <div key={slide.n} style={{ border: `1px solid ${BRAND.border}`, borderRadius: 8, overflow: "hidden" }}>
              ...
              <div style={{ padding: "14px 18px", display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                {slide.graphic && slide.graphic !== "none" && (
                  <p style={{ margin: 0, fontFamily: "'Lato', sans-serif", fontSize: 11, color: BRAND.textSecondary, fontStyle: "italic" }}>Graphic hint: {slide.graphic}</p>
                )}
              </div>
            </div>
          );
      ...
      {loading && (
        <div style={{ marginTop: 20, display: "flex", alignItems: "center", gap: 12, color: BRAND.textSecondary, fontFamily: "'Lato', sans-serif", fontSize: 13 }}>
          <Spinner />
          <span>Claude is writing the scripts and building the .pptx… (~20–40 seconds)</span>
        </div>
      )}
    </div>
  );
